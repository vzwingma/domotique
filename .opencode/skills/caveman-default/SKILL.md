...
> ⚠️ **Anti-duplication** : Ce skill est chargé automatiquement via `applyTo: "**"`. Les règles caveman sont aussi encodées dans `.opencode/copilot-instructions.md` (section `## 🗿 Mode communication`). **Ne jamais invoquer `/skill caveman` ou `/skill caveman-default` manuellement** — cela crée une duplication de ~3-5KB par invocation qui s'accumule à chaque tour suivant de la session.
